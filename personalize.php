...
function personalize_init() {
    ...
    load_textdomain('personalize', WP_LANG_DIR . '/print-science-designer-' . $locale . '.mo');
...
function admin_enqueue($hook) {
    wp_enqueue_style('print_science', plugins_url() . '/print-science-designer/css/admin.css');
...
function pz_scripts() {
	...
	wp_enqueue_script('function-slider', plugins_url() . '/print-science-designer/js/personal.js', array(), '1.0.0', true);
...
add_filter('woocommerce_before_cart_contents', 'cart_product_pz',1);
...
function cart_product_pz() {
    ...
    foreach ($woocommerce->cart->get_cart() as $cart_item_key => $values) {
        ...
        $custom_tab_options = array(
            'personalize' => get_post_meta($_product->id, 'personalize', true),
            'a_product_id' => get_post_meta($_product->id, 'a_product_id', true)
        );
        $arr_api_info = $wpdb->get_results('SELECT window_type FROM ' . $api_info_table . ' where id=1');
        $window_type = $arr_api_info[0]->window_type;
		
		//echo "<br>----".$cart_item_key;
		///echo "<pre>";
		//print_r($_SESSION);
		//echo "</pre>";
        $arr_types = array('New Window', 'Modal Pop-up window');
		if(!empty($values['variation_id'])){
			if(!isset($_SESSION['pro_' . $values['product_id'] . '_' . $values['variation_id'] . '_' . $cart_item_key])) {
				$_SESSION['pro_' . $values['product_id'] . '_' . $values['variation_id'] . '_' . $cart_item_key] = $_SESSION['sessionkey'];
			}
		}else{
			if(!isset($_SESSION['pro_' . $values['product_id'] . '_' . $cart_item_key])) {
				$_SESSION['pro_' . $values['product_id'] . '_' . $cart_item_key]= $_SESSION['sessionkey'];
			}
		}
		//echo "<pre>";
		//print_r($_SESSION);
		...
		if ($_product->exists() && $values['quantity'] > 0) {
            ...
            <tr class = "<?php echo esc_attr(apply_filters('woocommerce_cart_table_item_class', 'cart_table_item_print', $values, $cart_item_key)); ?>">
                ...
                <td class="product-thumbnail">
                    <?php
                    if ((isset($values['variation_id']) && $values['variation_id'] != '' && isset($_SESSION['pro_' . $values['product_id'] . '_' . $values['variation_id'] . '_' . $cart_item_key]) && $_SESSION['pro_' . $values['product_id'] . '_' . $values['variation_id'] . '_' . $cart_item_key] != '' && $custom_tab_options['personalize'] == 'y') || (isset($_SESSION['pro_' . $values['product_id'] . '_' . $cart_item_key]) && $_SESSION['pro_' . $values['product_id'] . '_' . $cart_item_key] != '' && $custom_tab_options['personalize'] == 'y')) {
...
function get_response_from_api($rtype, $sessionkey) {
    ...
    if ($sessionkey != '') {
        ...
        $imagePath = $previewUrls;

        $arr_return = array('pdf_file_url' => $pdf_file_url, 'image_path' => $imagePath);
    } 
	if ($rtype == 'imagepath') {
		echo '<div class="personalizationGallery">';
		foreach($imagePath as $imgUrl){
				echo '<a href="' . $imgUrl . '" data-rel="prettyPhoto['.$sessionkey.']" rel="prettyPhoto"><img src="' . $imgUrl . '"/></a>';
		}
		echo '</div>';
    }
    if ($rtype == 'pdf') {
...
remove_filter( 'woocommerce_add_cart_item_data', 'add_cart_item_data', 10, 2 );
...
function namespace_force_individual_cart_items($cart_item_data, $product_id, $variation_id=null) {
    global $woocommerce;
	...
	$outPrice = preg_replace("/[^0-9]+/","",$_REQUEST['totalPriceValue']);
	$cart_item_data['productPrice_'.$product_id] = number_format($outPrice/100, 2);
	$outPrice1 = preg_replace("/[^0-9]+/","",$_REQUEST['totalPriceValue1']);
	$cart_item_data['productPrice1_'.$product_id] = (number_format($outPrice1/100, 2)/$_REQUEST['quantity']);
    // $objcart 	= 		new WC_Cart();
    //$cart_id = WC_Cart::generate_cart_id($product_id, $variation_id, $variations, $cart_item_data);
	...
	foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) {
		if($cart_item['product_id'] == $product_id ){
			if ($variation_id == '') {
				...
				if(!isset($_SESSION['pro_' . $product_id . '_' . $variation_id . '_' . $cart_item_key])) {
					$_SESSION['pro_' . $product_id . '_' . $variation_id . '_' . $cart_item_key] = $_SESSION['sessionkey'];
				}
			}
		}
	}
...
function add_custom_price() {
	...
	foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $value ) {
		...
		$personalizeData = get_post_meta($proID, 'personalize', true);
		$product_addons =  get_product_addons($proID);
		if(($personalizeData == 'y') && (count($product_addons) != 0) && ($value['productPrice_'.$proID] != '0.00')){
			$value['productPrice1_'.$proID]/$value['quantity'];
			if($value['quantity'] == 1){
				$datanewValue = $value['productPrice_'.$proID];
				$typeUpdated = 1;
			}else if($value['quantity'] > 1){
  			  if($reffererUrl!='' && $reffererUrl != $cart_url){
				$quantityPrice = $value['productPrice_'.$proID] / $value['quantity'];
			    $typeUpdated = 2;
				$datanewValue = $quantityPrice;			
			  } else {
				//echo "<br>sss--".$value['data']->price ."---". $value['productPrice1_'.$proID];
				$updatePrice = $value['data']->price + $value['productPrice1_'.$proID];
				$typeUpdated = 3;
				$datanewValue = $updatePrice;
				  
			  }
		    }
			$value['data']->price = $datanewValue;
		}   
    }
}
